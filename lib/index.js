...
const setTrayIconForWaterLevel = (waterLevel, appIcon = null) => {
  console.log(Math.floor(new Date().getSeconds() / 12)) % 5;
  const embankmentHeight = 5;
  const currentWaterLevel = (Math.floor(new Date().getSeconds() / 12)) % 5;
...
module.exports = class Lib {
  constructor({ country, prefecture, river }, host, env) {
    ...
    this.river = river;
    this.list = [
      { country: 'taiwn', prefecture: 'taipei', river: 'danshui' },
